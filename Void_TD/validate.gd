...
func _init() -> void:
	...
	_assert(GameConfig.STARTING_CREDITS == 250, "STARTING_CREDITS = 250")
	...
	_assert(boss_stats["hp"] == 2500, "Boss hp = 2500")
	...
	_assert(w2_count == 10, "Wave 2: 10 enemies (6+4)")
	var w3_count = 0
	for g in waves[2].groups: w3_count += g.count
	_assert(w3_count == 12, "Wave 3: 12 enemies (5+6+1 boss)")
	...
	_assert(enemy.max_hp == 80.0, "Scout max_hp = 80")
	_assert(enemy.speed == 210.0, "Scout speed = 210")
	...
	_assert(enemy.current_hp == 40.0, "HP reduced by damage")
	_assert(not enemy.is_dead, "Not dead at 40 HP")
	enemy.take_damage(40.0)
	...
	_assert(tank.max_hp == 700.0, "Tank max_hp = 700")
	...
	_assert(boss.max_hp == 2500.0, "Boss max_hp = 2500")
	...
	_assert(scaled.max_hp == 160.0, "Scout at scale 2.0 has 160 hp")
